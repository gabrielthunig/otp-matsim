...
public class RouteMatrixCalculator {
	...
	public RouteMatrixCalculator(PathService pathservice, TransitSchedule transitSchedule, 
	...
	/*
	 * input: List of Facilities, time in seconds from midnight
	 */
	public double[][] calcMatrix(List<Facility> facilities, double time) {
		double[][] result = new double[facilities.size()][facilities.size()];
		for (int i = 0; i < facilities.size(); i++) {
			for (int j = 0; j < facilities.size(); j++) {
				result[i][j] = calcRoute(facilities.get(i), facilities.get(j), time).get(1).getTravelTime();
			}
		}
		return result;
	}
...
//	XXX: Why not let the people walk from spawned points to stations but suppose they start and end at them?
